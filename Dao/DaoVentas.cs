...
namespace Dao
{
    public class DaoVentas
    {
        ...
        public DataTable ObtenerVentas()
        {
            ...
            string srtSQL = "SELECT ven_codigo,ven_usuarios_dni, usu_apellido + ', '+ usu_nombre AS usuario, ven_medio_pago_codigo, mp_nombre, ven_fecha,ven_fecha_requerida,ven_fecha_envio,ven_total_facturado,ven_codigo_estado, est_nombre FROM Ventas INNER JOIN Usuarios ON ven_usuarios_dni=usu_dni INNER JOIN Medios_de_pago ON ven_medio_pago_codigo = mp_codigo INNER JOIN Estados ON ven_codigo_estado=est_codigo";
